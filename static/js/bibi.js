...
let items = []
let page = 1
let Url = 'https://kkapi.fomal.cc/api/ispeak?author=6319fedef46fae97dcfa5ee2&page=' // 修改api，记得带参数page
...
function getNew() {
    ...
    bibi.innerHTML += '<div id="loading"><img src="/assets/loading3.gif" alt="loading"></div>' // loading图片可以f12在我网站源码下载，也可以使用其他图片。
    ...
    fetch(Url + page).then(res => res.json()).then((res) => {
        ...
        nowNum += items.length
        if (page == 1) {
            document.querySelector('.bb-info').innerHTML = '<i class="far fa-comment-alt"></i> 我的唠叨(' + total + ')'
        }
        page += 1
    }).then(() => {
        bb();
